<template>
  <v-container fluid class="page">
      ...
      <v-col v-for="item in sources" :key="item.signal_source_id" cols="12" md="6" xl="4">
        <v-card class="source-card" elevation="0">
          <v-card-text>
            ...
            <p>{{ item.config.analysis_mode === 'shared_reference' ? '引用共享 AI 运行数据，不重复调用模型' : `${item.config.model || '未选择模型'} · 每 ${item.config.analysis_interval_minutes || '-'} 分钟分析` }}</p>
    ...
    <v-dialog v-model="dialog" max-width="820" persistent><v-card>
      ...
      <v-card-text><v-row>
        <v-col cols="12" md="6"><v-text-field v-model="form.name" label="名称"/></v-col><v-col cols="12" md="3"><v-text-field v-model="form.symbol" label="交易品种"/></v-col><v-col cols="12" md="3"><v-select v-model="form.period" :items="periods" label="分析周期"/></v-col>
        <v-col cols="12"><v-btn-toggle v-model="form.config.analysis_mode" mandatory color="success" variant="outlined" divided><v-btn value="self_analysis">自主 AI 分析</v-btn><v-btn value="shared_reference">引用共享数据</v-btn></v-btn-toggle></v-col>
        <template v-if="form.config.analysis_mode === 'self_analysis'"><v-col cols="12" md="6"><v-select v-model="form.config.model" :items="options.models" label="运行模型"/></v-col><v-col cols="12" md="6"><v-select v-model.number="form.config.analysis_interval_minutes" :items="intervalOptions" label="调用间隔（分钟）"/></v-col><v-col cols="12" md="6"><v-text-field v-model.number="form.config.kline_count" label="分析 K 线数量" type="number" min="10" max="500"/></v-col><v-col cols="12" md="6"><v-text-field v-model.number="form.config.min_confidence" label="最低置信度" suffix="%" type="number" min="0" max="100"/></v-col><v-col cols="12"><v-textarea v-model="form.config.system_prompt" label="系统提示词" rows="3" auto-grow/></v-col><v-col cols="12"><v-textarea v-model="form.config.analysis_prompt_template" label="分析提示词模板" rows="6" auto-grow/></v-col><v-col cols="12"><v-switch v-model="form.share_runtime_data" color="success" inset label="共享运行数据" hint="仅共享分析结论和可见参数，不共享提示词与模型配置。" persistent-hint/></v-col></template>
        <template v-else><v-col cols="12"><v-select v-model="form.config.shared_runtime_id" :items="sharedOptions" label="选择共享 AI 数据" :loading="loadingOptions"/><v-alert type="info" variant="tonal">引用型信号源不调用大模型，持续消费共享者的最新分析结果。</v-alert></v-col></template>
      </v-row></v-card-text>
...
const options = reactive({ models: [], shared: [], system: '', prompt: '' })
const emptyForm = () => ({ signal_source_id: '', name: '', symbol: '', period: 'M5', enabled: true, share_runtime_data: false, config: { analysis_mode: 'self_analysis', model: '', analysis_interval_minutes: 5, kline_count: 100, min_confidence: 70, system_prompt: '', analysis_prompt_template: '', shared_runtime_id: '', reference_runtime_ids: [] } })
const form = reactive(emptyForm())
const intervalOptions = computed(() => [1,2,3,5,10,15,30,60,120,240,480,720,1440].filter(x => x >= ({M1:1,M5:5,M15:15,H1:60,H4:240}[form.period] || 1)).map(value => ({ title: `${value} 分钟`, value })))
const sharedOptions = computed(() => options.shared.map(item => ({ value: item.share_id, title: `${item.symbol} · ${item.period} · ${item.owner_username} · 置信度 ${item.result?.overall_trend?.confidence ?? '-'}` })))
...
async function save () { saving.value=true; try { const data=clone(form); if (data.config.analysis_mode==='self_analysis') { data.config.analysis_interval_minutes=Math.max(data.config.analysis_interval_minutes || 0, ({M1:1,M5:5,M15:15,H1:60,H4:240}[data.period]||1)) } const result=data.signal_source_id ? await marketAPI.updateAISignalSource(data.signal_source_id,data) : await marketAPI.createAISignalSource(data); dialog.value=false; messageType.value='success'; message.value=result.source?.locked ? '已保存并冻结' : 'AI 信号源已保存'; await load() } catch(e) { messageType.value='error'; message.value=e.response?.data?.detail || '保存失败' } finally { saving.value=false } }
...
watch(() => form.period, () => { const min={M1:1,M5:5,M15:15,H1:60,H4:240}[form.period]||1; form.config.analysis_interval_minutes=Math.max(Number(form.config.analysis_interval_minutes)||0,min) })
onMounted(load)
</script>
