...
AITrader 把 **MT5 行情接入、AI 多周期研判、可配置策略、风险控制、账户级自动执行和交易执行** 串成一条完整链路。
...
- **账户级执行控制**：策略命中并通过风控后自动进入执行链路；用户通过账户部署、暂停、停止和交易开关控制运行。
...
策略触发后自动生成交易执行请求。用户可以在账户层暂停、停止或关闭自动交易，策略本身不再维护独立启用开关。
